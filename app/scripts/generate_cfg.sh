...
# Usage: ./generate_cfg.sh <project_path> <function_name> <output_dir> [cpg_path]
...
DOT_FILE="$OUTPUT_DIR/${FUNCTION_NAME}_cfg.dot"
PNG_FILE="$OUTPUT_DIR/${FUNCTION_NAME}_cfg.png"

# Create temporary script for Joern
SCRIPT_FILE="/tmp/cfg_$$.sc"
cat <<EOF > "$SCRIPT_FILE"
importCpg("$CPG_PATH")
cpg.method.name("$FUNCTION_NAME").dotCfg.l |> "$DOT_FILE"
EOF
